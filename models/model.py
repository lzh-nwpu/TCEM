...
class Model(nn.Module):
    ...
    def forward(self, code_x, divided, neighbors, lens, time_weight):
        embeddings = self.embedding_layer()
        c_embeddings, n_embeddings, u_embeddings = embeddings

        output = []
        for code_x_i, divided_i, neighbor_i, len_i, time_weight_i in zip(code_x, divided, neighbors, lens, time_weight):
            # np.savetxt('code_x_i.csv', code_x_i.int().cpu().numpy())
            # df = pd.DataFrame(code_x_i.int().cpu().numpy())
            # df.to_csv('data.csv', index=False)
            no_embeddings_i_prev = None
            output_i = []
            h_t = None
            for t, (c_it, d_it, n_it, len_it) in enumerate(zip(code_x_i, divided_i, neighbor_i, range(len_i))):
                co_embeddings, no_embeddings = self.graph_layer(c_it, n_it, c_embeddings, n_embeddings, time_weight_i)
