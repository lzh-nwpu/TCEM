...
from preprocess.auxiliary import generate_code_code_adjacent, generate_neighbors, normalize_adj, divide_middle
...
if __name__ == '__main__':
    ...
    if from_saved:
        patient_admission = pickle.load(open(os.path.join(parsed_path, 'patient_admission_b.pkl'), 'rb'))
        admission_codes = pickle.load(open(os.path.join(parsed_path, 'admission_codes_b.pkl'), 'rb'))
    else:
    ...
    pickle.dump(normalize_adj(code_adj), open(os.path.join(os.path.join(dataset_path, 'standard'), 'code_adj.pkl'), 'wb'))
    ...
    (train_code_x, train_codes_y, train_visit_lens, train_code_x_timestamp) = build_code_xy(train_pids, *common_args)
    print('building valid codes features and labels ...')
    (valid_code_x, valid_codes_y, valid_visit_lens, valid_code_x_timestamp) = build_code_xy(valid_pids, *common_args)
    print('building test codes features and labels ...')
    (test_code_x, test_codes_y, test_visit_lens, test_code_x_timestamp) = build_code_xy(test_pids, *common_args)
    ...
    save_data(train_path, train_code_x, train_visit_lens, train_codes_y, train_hf_y, train_divided, train_neighbors, train_code_x_timestamp)
    print('\tsaving valid data')
    save_data(valid_path, valid_code_x, valid_visit_lens, valid_codes_y, valid_hf_y, valid_divided, valid_neighbors, valid_code_x_timestamp)
    print('\tsaving test data')
    save_data(test_path, test_code_x, test_visit_lens, test_codes_y, test_hf_y, test_divided, test_neighbors, test_code_x_timestamp)
