...
class EHRDataset:
    def __init__(self, data_path, label='m', batch_size=32, shuffle=True, device=torch.device('cpu')):
        super().__init__()
        self.path = data_path
        self.time_weight = load_sparse(os.path.join(os.path.dirname(self.path), 'time_weight.npz'))
    ...
    def __getitem__(self, index):
        ...
        time_weight = torch.from_numpy(self.time_weight[slices]).to(device).float()
        return code_x, visit_lens, divided, y, neighbors, time_weight

    # def __getitem__(self, index):
    #     device = self.device
    #     code_x = torch.from_numpy(self.code_x[index]).to(device)
    #     visit_lens = torch.tensor(self.visit_lens[index]).to(device=device, dtype=torch.long)
    #     y = torch.from_numpy(self.y[index]).to(device=device, dtype=torch.float32)
    #     divided = torch.from_numpy(self.divided[index]).to(device)
    #     neighbors = torch.from_numpy(self.neighbors[index]).to(device)
    #     time_weight = torch.from_numpy(self.time_weight[index]).to(device).float()
    #     return code_x, visit_lens, divided, y, neighbors, time_weight
