...
def evaluate_codes(model, dataset, loss_fn, output_size, historical=None):
    ...
    for step in range(len(dataset)):
        ...
        code_x, visit_lens, divided, y, neighbors, time_weight = dataset[step]
        output = model(code_x, divided, neighbors, visit_lens, time_weight)
...
def evaluate_hf(model, dataset, loss_fn, output_size=1, historical=None):
    ...
    preds = []
    for step in range(len(dataset)):
    # for step, (code_x, visit_lens, divided, y, neighbors, time_weight) in enumerate(dataset):
        code_x, visit_lens, divided, y, neighbors, time_weight = dataset[step]
        output = model(code_x, divided, neighbors, visit_lens, time_weight).squeeze()
        loss = loss_fn(output, y)
        total_loss += loss.item() * output_size * len(code_x)
